...
<body>
    ...
    <script>
        ...
        // Pengecekan status waktu sholat secara real-time dari rotator utama
        async function checkPrayerMode() {
            try {
                ...
                if (data.active && data.phase === 'countdown') {
                    ...
                    // Cari halaman utama-embed di list aktif
                    const utamaIndex = activePages.findIndex(p => p.url && p.url.includes('utama-embed'));
                    if (utamaIndex !== -1) {
                        // Jika saat ini layar sedang di halaman lain (misal: keuangan), langsung pindah ke Jadwal Sholat!
                    ...
                    // Lepas kunci rotasi jika waktu sholat sudah selesai
        ...
        // Cek status sholat dan pengaturan rotasi setiap 2 detik agar responsif
        setInterval(checkPrayerMode, 2000);
