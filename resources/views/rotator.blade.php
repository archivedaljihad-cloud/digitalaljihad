...
<body>
    ...
    <script>
        ...
        // Cek apakah rotasi sedang dikunci oleh sholat aktif
        ...
        function resetCountdown() {
            ...
            if (rotationEnabled && activePages.length > 1) {
                countdownIntervalId = setInterval(() => {
                    if (isPrayerActive()) {
                        const utamaIndex = activePages.findIndex(p => p.url && p.url.includes('utama-embed'));
                        if (utamaIndex !== -1 && currentIndex !== utamaIndex) {
                            currentIndex = utamaIndex;
                            loadPage(currentIndex);
                        }
                        return; // Jeda hitung mundur rotasi
                    }
        ...
        // Pengecekan waktu sholat secara lokal berdasarkan cache jadwal harian dari localStorage
        function checkPrayerModeLocally() {
            try {
                const now = new Date();
                const currentTotalMinutes = (now.getHours() * 60) + now.getMinutes();

                const cachedSchedules = JSON.parse(localStorage.getItem('todayPrayerTimes') || '[]');
                let isTimeToLock = false;

                for (const prayer of cachedSchedules) {
                    const [h, m] = prayer.time.split(':').map(Number);
                    const jadwalTotalMinutes = (h * 60) + m;
                    const startMinutes = jadwalTotalMinutes - 2; // 2 menit sebelum sholat (misal 03:58 untuk jam 04:00)
                    const endMinutes = jadwalTotalMinutes + 30;   // 30 menit sesudah sholat
                ...
                console.error('Local Prayer Mode Check Failed:', error);
            }
        }
        ...
        // Cek waktu sholat dan pengaturan rotasi setiap detik agar sangat responsif
        setInterval(checkPrayerModeLocally, 1000);
        setInterval(fetchLatestSettings, 5000);

        checkPrayerModeLocally();
