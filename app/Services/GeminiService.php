...
class GeminiService
{
    ...
    public function __construct()
    {
        ...
        $this->apiKey = $this->setting?->gemini_api_key ?? env('GEMINI_API_KEY') ?? null;
        $this->model = $this->setting?->gemini_model ?? 'gemini-1.5-flash';
    }
    ...
    /**
     * Menguji koneksi ke Google Gemini API.
     */
    public function testConnection(): array
    {
        if (!$this->hasApiKey()) {
            return [
                'success' => false,
                'message' => 'API Key belum dikonfigurasi. Silakan masukkan Gemini API Key terlebih dahulu.'
            ];
        }
        ...
        try {
            ...
            if ($response->successful()) {
                return [
                    'success' => true,
                    'message' => 'Koneksi ke Google Gemini API (' . $this->model . ') BERHASIL!'
                ];
            }

            $errorMsg = $response->json('error.message') ?? 'Gagal menghubungi API Google Gemini (HTTP ' . $response->status() . ').';
            return [
                'success' => false,
                'message' => $errorMsg
            ];
    ...
    /**
     * Menghasilkan teks pengumuman terstruktur dari poin-poin mentah.
     */
    public function generateAnnouncement(string $rawPoints, string $category = 'Kajian Rutin', string $namaMasjid = 'Masjid Jami Al-Jihad'): array
    {
        if (!$this->hasApiKey()) {
            return $this->fallbackAnnouncement($rawPoints, $category, $namaMasjid);
        }

        $prompt = "Kamu adalah asisten sekretaris DKM {$namaMasjid} yang sangat santun, profesional, dan berbahasa Indonesia Islami yang baik.
...
Tolong susun draf pengumuman resmi masjid dan kembalikan HANYA dalam format JSON valid tanpa format markdown code block ```json:
{
  ...
  \"tempat\": \"Tempat pelaksanaan (default: Ruang Utama {$namaMasjid})\",
  ...
  \"running_text\": \"Versi ringkas 1 kalimat padat untuk teks berjalan layar TV (maksimal 130 karakter)\"
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                if (is_array($parsed) && isset($parsed['judul'], $parsed['isi'])) {
                    return [
                        'success' => true,
                        'data'    => $parsed,
                        'source'  => 'gemini_ai'
            ...
            Log::warning('Gemini generateAnnouncement failed: ' . $e->getMessage());
        }
    ...
    /**
     * Mengambil hadits & mutiara hikmah harian.
     */
    public function getDailyHikmah(bool $forceRefresh = false): array
    {
        $today = Carbon::today('Asia/Jakarta');
        $todayDateString = $today->format('Y-m-d');
        $dayName = $today->locale('id')->isoFormat('dddd');

        // Cek cache tersimpan di database jika tidak di-force
        if (!$forceRefresh && $this->setting) {
            try {
                if (Schema::hasColumn('app_settings', 'daily_hikmah_date') && Schema::hasColumn('app_settings', 'daily_hikmah_cache')) {
                    if ($this->setting->daily_hikmah_date && Carbon::parse($this->setting->daily_hikmah_date)->format('Y-m-d') === $todayDateString && !empty($this->setting->daily_hikmah_cache)) {
                        $cachedData = json_decode($this->setting->daily_hikmah_cache, true);
                        if (is_array($cachedData) && isset($cachedData['arab'])) {
                            return $cachedData;
                        }
                    }
                }
        ...
        // Jika ada API Key, request ke Gemini
        if ($this->hasApiKey()) {
            $prompt = "Sebagai ahli hadits dan sastra Islam, pilihkan TEPAT 1 (SATU) Hadits Shahih otentik (dari Shahih Bukhari, Shahih Muslim, Sunan Abu Dawud, Tirmidzi, atau An-Nasa'i) yang sangat relevan untuk hari {$dayName}.
...
Kembalikan HANYA dalam format JSON valid tanpa format markdown code block ```json:
{
  ...
  \"hari\": \"{$dayName}\",
  \"tanggal\": \"{$today->locale('id')->isoFormat('D MMMM Y')}\"
}";
            ...
            try {
                ...
                if ($response->successful()) {
                    $content = $response->json('candidates.0.content.parts.0.text');
                    $parsed = json_decode(trim($content), true);
                    if (is_array($parsed) && isset($parsed['arab'], $parsed['terjemahan'])) {
                        $this->saveHikmahCache($parsed, $todayDateString);
                        return $parsed;
                    }
                }
            } catch (\Throwable $e) {
                Log::warning('Gemini getDailyHikmah failed: ' . $e->getMessage());
            }
        }

        // Fallback offline hadits otentik per hari
        $offlineData = $this->getOfflineHadithByDay($today->dayOfWeek);
        $offlineData['hari'] = $dayName;
        $offlineData['tanggal'] = $today->locale('id')->isoFormat('D MMMM Y');
        $this->saveHikmahCache($offlineData, $todayDateString);
        return $offlineData;
    }

    /**
     * Simpan cache hikmah harian ke database.
     */
    protected function saveHikmahCache(array $data, string $dateString): void
    {
        if (!$this->setting) return;
        try {
            if (Schema::hasColumn('app_settings', 'daily_hikmah_cache') && Schema::hasColumn('app_settings', 'daily_hikmah_date')) {
                $this->setting->daily_hikmah_cache = json_encode($data);
                $this->setting->daily_hikmah_date = $dateString;
                $this->setting->save();
            }
        } catch (\Throwable $e) {}
    }
    ...
    /**
     * Koleksi Hadits Shahih Bawaan (Fallback Offline Anti-Gagal)
     */
    protected function getOfflineHadithByDay(int $dayOfWeek): array
    {
        $collection = [
            // 0: Ahad
            0 => [
        ...
        return $collection[$dayOfWeek] ?? $collection[5];
    }

    /**
     * Generator Pengumuman Fallback Offline
     */
    protected function fallbackAnnouncement(string $rawPoints, string $category, string $namaMasjid): array
    {
        $judul = ucwords(trim($rawPoints));
        if (strlen($judul) > 60) {
            $judul = substr($judul, 0, 57) . '...';
        }
        ...
        return [
            ...
            'data' => [
                ...
                'isi'          => "Assalamu'alaikum Warahmatullahi Wabarakatuh.\n\nHadirilah kegiatan {$category} di {$namaMasjid}. Poin kegiatan: {$rawPoints}.\n\nMari kita ramaikan majelis kebaikan ini bersama keluarga. Semoga Allah SWT memudahkan langkah kita. Barakallahu fiikum.",
